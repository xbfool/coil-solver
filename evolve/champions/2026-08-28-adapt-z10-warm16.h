...
static inline int policy_wp_grow(int p)   { return p + p / 16 + 1; }  // 保温(已证 0.7648): 放疏减半

// ---- 演绎武器默认开关/密度（0=关；生产配方 BJ=500 只在 env 显式给出时生效）----
#define POLICY_BJ_DEFAULT     500 // 叠加实验: 固定节拍兜自调退避盲区?
#define POLICY_ADAPT_DEFAULT  1   // 自适应武器组(已证: 独占核复评 0.598→0.704)
#define POLICY_RESTART_DEFAULT 0  // Luby 随机重启轮

// ---- 探针预算（第一层证伪漏斗）----
#define POLICY_PROBE_ZERO 10      // 近全撤: 零信息族探针层存废实验
