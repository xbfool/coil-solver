// Coil solver — v60: 割点剪枝实验（≥3块死区）。v58不动。
//
// 以下是 v53 的说明。
...
// 精确连通性：剩余格必须都能从 p 走到
// 割点剪枝：自由区(g==1)里若存在顶点去掉后分>=3连通块 => 无哈密顿路径 => 死。
// 迭代Tarjan，从自由入口 e0 遍历整个(连通的)自由区。返回1=死,0=通过。
static int *dt_, *lw_, *par_, *cidx_, *cutc_, *stk_;
long long artic_fire = 0, artic_call = 0;
static int artic_dead(int e0) {
    ++seen_id;
    int timer = 1, sp = 0;
    stk_[0] = e0; par_[e0] = -1; cidx_[e0] = 0; cutc_[e0] = 0;
    seen[e0] = seen_id; dt_[e0] = lw_[e0] = timer++;
    int root_children = 0, dead = 0;
    while (sp >= 0 && !dead) {
        int u = stk_[sp];
        int advanced = 0;
        while (cidx_[u] < 4) {
            ...
            if (!g[v]) continue;
            if (seen[v] != seen_id) {
                seen[v] = seen_id; dt_[v] = lw_[v] = timer++; par_[v] = u; cidx_[v] = 0; cutc_[v] = 0;
                stk_[++sp] = v; advanced = 1; if (u == e0) ++root_children;
                break;
            } else if (v != par_[u] && dt_[v] < lw_[u]) lw_[u] = dt_[v];
        }
        if (advanced) continue;
        --sp;
        int pu = par_[u];
        if (pu >= 0) {
            if (lw_[u] < lw_[pu]) lw_[pu] = lw_[u];
            if (lw_[u] >= dt_[pu]) {                 // pu 割点，u 子树被切
                if (pu != e0) { if (++cutc_[pu] + 1 >= 3) { dead = 1; } }   // 非根：切子树数+1(上方)>=3
            }
        }
    }
    if (!dead && root_children >= 3) dead = 1;        // 根去掉分>=3块
    return dead;
}
...
static int dfs(int p, int remaining, int depth) {
    ...
    { static int artic_on = -1; if (artic_on < 0) artic_on = getenv("ARTIC") ? atoi(getenv("ARTIC")) : 0;
      if (artic_on && remaining > 2) {
        int e0 = -1; for (int d = 0; d < 4; ++d) { int v = p + delta[d]; if (g[v]) { e0 = v; break; } }
        if (e0 >= 0) { ++artic_call; if (artic_dead(e0)) { ++dth_reach; ++artic_fire; return 0; } } } }
...
int main(int argc, char **argv) {
    ...
    dt_ = malloc(sizeof(int)*(size_t)N); lw_ = malloc(sizeof(int)*(size_t)N); par_ = malloc(sizeof(int)*(size_t)N); cidx_ = malloc(sizeof(int)*(size_t)N); cutc_ = malloc(sizeof(int)*(size_t)N); stk_ = malloc(sizeof(int)*(size_t)N);
  ...
  child_work:;
    ...
    for (int i0 = 0; i0 < keep; ++i0) {
        ...
        if (getenv("ARTIC")) fprintf(stderr, "ARTICSTAT fire=%lld call=%lld\n", artic_fire, artic_call);
