// Coil solver — v46: 强制步快速通道（唯一候选免洪水；被迫的步占绝大多数时省掉 95% 热点的大头）
//
// 以下是 v45 的说明。
...
static int dfs(int p, int remaining, int depth) {
    ...
    // ===== 强制步快速通道（v46）=====
    // L413 全局定死 71% 的边 —— 搜索里绝大多数步是被迫的（estate_ok 把另外三个方向
    // 直接否掉），但每个被迫步仍要付一次 reach_local（~2μs，gprof 实测占 95% 时间）。
    // 先用便宜的 estate_ok + cheap_ok 过一遍四个方向：**恰好一个**通过时直接走，
    // 跳过 reach_local / propagate_dyn / 排序。正确性不变 —— 这步反正必须走，
    // 连通性真坏了会在下一个分叉点被抓住，只是晚了几步；换来的是强制链上每格省一次洪水。
    {
        int fnc = 0, fd = -1, fend = 0, flen = 0;
        for (int d = 0; d < 4 && fnc < 2; ++d) {
            int dd = delta[d];
            if (!g[p + dd]) continue;
            int c = p, len = 0;
            while (g[c + dd]) { c += dd; ++len; }
            if (!estate_ok(p, d, dd, len, c)) continue;
            for (int k = 0, q = p; k < len; ++k) { q += dd; mark(q); }
            int rem2 = remaining - len;
            if (rem2 == 0 || cheap_ok(c, rem2)) { ++fnc; fd = d; fend = c; flen = len; }
            for (int i = 0, back = c; i < len; ++i, back -= dd) unmark(back);
        }
        if (fnc == 0) return 0;
        if (fnc == 1) {                                    // 被迫步：直接走
            int dd = delta[fd];
            for (int k = 0, q = p; k < flen; ++k) { q += dd; mark(q); }
            path[path_len++] = DCH[fd];
            int r = dfs(fend, remaining - flen, depth + 1);
            if (r) return r;
            --path_len;
            for (int k = 0, back = fend; k < flen; ++k, back -= dd) unmark(back);
            return 0;
        }
    }
