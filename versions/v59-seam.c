// Coil solver — v59: v58 + 缝合线剪枝（零类解是近闭环：19/19 实测终点距起点 L1≤21，17/19 ≤8）
//
// 以下是 v53 的说明。
...
static int seed_mode = 1;
static int seam_r;         // v59：>0 时第三层强制终点落在起点 L1 半径内（零类 portfolio 分片）  // v56 portfolio：0=v52语义(空底全播) 1=v54(底座滑播) 2=v55(底座全播)，按 shard%3 混编
...
int main(int argc, char **argv) {
    ...
    if (getenv("FORCESHARD") && gfix_total == 0) seam_r = getenv("SEAMR") ? atoi(getenv("SEAMR")) : ((force_shard_identity % 4 == 1) ? 10 : (force_shard_identity % 4 == 3) ? 30 : 0);
    ...
    if (nshard > 1) {
        for (int k = 0; k < nshard; ++k) {
            ...
            if (pid == 0) {                                 // 子进程
                ...
                if (gfix_total == 0) seam_r = getenv("SEAMR") ? atoi(getenv("SEAMR")) : ((k % 4 == 1) ? 10 : (k % 4 == 3) ? 30 : 0);
                goto child_work;
            }
            kids[k] = pid;
            close(pfd[k * 2 + 1]);
  ...
  child_work:;
    ...
    for (int i0 = 0; i0 < keep; ++i0) {
        ...
        if (seam_r > 0) {                          // v59 缝合线剪枝：终点候选 = 起点邻域（∩ 原候选）
            int sx0 = s % W, sy0 = s / W, cnt2 = 0;
            for (int c2 = 0; c2 < N; ++c2) {
                if (!g[c2]) { tcand[c2] = 0; continue; }
                int ax = c2 % W - sx0; if (ax < 0) ax = -ax;
                int ay = c2 / W - sy0; if (ay < 0) ay = -ay;
                int in = (ax + ay <= seam_r);
                if (tc_on && !tcand[c2]) in = 0;
                tcand[c2] = (unsigned char)in;
                if (in) ++cnt2;
            }
            tcand_for = s;
            if (cnt2 == 0) { tc_on = 0; live_end = -1; continue; }   // 邻域全灭：本分片放弃该起点
            tc_on = 1; live_end = cnt2; ntc_low = 0;
            for (int c2 = 0; c2 < N; ++c2)
                if (g[c2] && deg[c2] <= 1 && !tcand[c2]) ++ntc_low;
        }
