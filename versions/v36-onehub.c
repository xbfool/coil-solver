...
#define OKAB(c) (rstA < 0 || blk[c] == rstA || blk[c] == rstB)
    if (c0 == c1) {
        // 偶数格：两端点异色。原来挂**两个**独立哑点（各容量 1），每个都是连向该色全部格子的 hub，
        // 两个 hub 让残量图连通性翻倍，SCC 糊成一整块 => 实测 26% 的偶数关完全过滤不动。
        // 改成**一个「穿透」哑点**：左格 -> D_in -> D_out -> 右格，中间容量 1。
        // 语义完全等价（恰好一个左格、一个右格各少用一条真实边），但 hub 减半，
        // 且两端天然耦合在同一个单位上 —— 这正是原来缺的那个耦合。
        // 源汇不用额外接：那一个单位仍然从源流出、流入汇，两侧总量自动配平。
        for (int c = 0; c < N; ++c) if (g0[c] && !col[c] && OKB(c)) fadd(FCELL(c), FDUM, 1);
        fadd(FDUM, FDUM2, 1);
        for (int c = 0; c < N; ++c) if (g0[c] && col[c] && OKA(c)) fadd(FDUM2, FCELL(c), 1);
