...
int main(int argc, char **argv) {
    ...
    if (getenv("PROBEDUMP")) {
        ...
        for (int s0 = 0; s0 < N; ++s0) {
            ...
            // v72：逐起点吐一组候选排序特征（不只 F）。
            // 动机：针位测量证明 F 的百分位平均 46%，与均匀随机无法区分 ——
            // 排序信号是假的。有银行里 800 个已知解当标注集，问题就变成
            // "哪个特征能把针排到前面"，那就得先把候选特征都吐出来。
            // end_cand / minL1 只在 STRONG=1（跑过 do_flow）时有意义。
            int deg0 = 0, nfix_s = 0;
            for (int d = 0; d < 4; ++d) {
                if (!g0[s0 + delta[d]]) continue;
                ++deg0;
                if (estate[s0 * 4 + d] == 1) ++nfix_s;
            }
            int minL1 = -1;
            if (strongp && tcand_for == s0) {
                int sx = s0 % W, sy = s0 / W, best = 1 << 30;
                for (int c = 0; c < N; ++c) {
                    if (!g0[c] || !tcand[c] || c == s0) continue;
                    int dx = c % W - sx, dy = c / W - sy;
                    int l1 = (dx < 0 ? -dx : dx) + (dy < 0 ? -dy : dy);
                    if (l1 < best) best = l1;
                }
                minL1 = (best == (1 << 30)) ? -1 : best;
            }
            // OK <格号> <F> <度数> <起点必用边数> <终点候选数> <到最近终点候选的L1>
            printf("OK %d %lld %d %d %lld %d\n", s0, fs, deg0, nfix_s,
                   strongp ? end_cand : -1, minL1);
