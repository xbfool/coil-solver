// Coil solver — v8: 把洪水填充换成 Tarjan，一次遍历顺带拿到割点，剪掉大块
//
...
// 一次 Tarjan：连通性 + 割点 + 死端。任一条不满足直接判死。
// 图 = 剩余未访问格 ∪ {当前位置 p}；p 作为根，路径必须从 p 出发覆盖全部剩余格。
static int *disc, *low, *cutc, *stk_v, *stk_p, *stk_d;

static int artic_ok(int p, int rem) {
    ++seen_id;
    int timer = 0, count = 0, dead = 0, root_kids = 0, sp = 0;
    seen[p] = seen_id; disc[p] = low[p] = timer++; cutc[p] = 0;
    stk_v[0] = p; stk_p[0] = -1; stk_d[0] = 0; sp = 1;

    while (sp) {
        int v = stk_v[sp - 1], par = stk_p[sp - 1], d = stk_d[sp - 1];
        if (d < 4) {
            ++stk_d[sp - 1];
            int u = v + delta[d];
            if (u != p && !g[u]) continue;          // 墙或已访问（p 例外：它是图里的根）
            if (seen[u] != seen_id) {
                seen[u] = seen_id; disc[u] = low[u] = timer++; cutc[u] = 0;
                stk_v[sp] = u; stk_p[sp] = v; stk_d[sp] = 0; ++sp;
            } else if (u != par && disc[u] < low[v]) {
                low[v] = disc[u];                    // 回边
            }
        } else {
            --sp;
            if (v != p) {
                ++count;
                int deg = 0;                         // 死端统计：p 相邻的格子能从 p 进来，度 +1
                for (int k = 0; k < 4; ++k) {
                    int n = v + delta[k];
                    if (g[n] || n == p) ++deg;
                }
            ...
            if (par >= 0) {
                ...
                if (par == p) {
                    if (++root_kids >= 2) return 0;  // 端点却是割点 —— 只能往一个方向走
                } else if (low[v] >= disc[par]) {
                    if (++cutc[par] >= 2) return 0;  // 割点切出 >=3 块（含父侧那块）
                }
            }
        }
    }
...
static int dfs(int p, int remaining) {
    ...
    for (int d = 0; d < 4; ++d) {
        ...
        if (rem2 == 0 || (cheap_ok(c, rem2) && artic_ok(c, rem2))) {
...
int main(int argc, char **argv) {
    ...
    disc = malloc(sizeof(int) * (size_t)N); low = malloc(sizeof(int) * (size_t)N);
    cutc = malloc(sizeof(int) * (size_t)N);
    stk_v = malloc(sizeof(int) * (size_t)(N + 4)); stk_p = malloc(sizeof(int) * (size_t)(N + 4));
    stk_d = malloc(sizeof(int) * (size_t)(N + 4));
