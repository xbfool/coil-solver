// Coil solver — v29: 等价推理（边之间的 EQUAL/OPPOSITE 关系）—— 专门产「禁用」的引擎
//
// 诊断：我们判定的边**必用 : 禁用 ≈ 7 : 1**，而真解里约 29% 的边不被使用。
// 调研指出根源：「必用」来自向上计数到界，而「禁用」只有三个来源
// （奇偶代数 / 割分离子 / 假设-反驳），我们一条都没有。
// 日本人做 Slitherlink 把这两类分得很清：**「最大数理論」产线，「通過点理論」产 ×**。
//
// 这一版实现的就是产 × 那一半：不追求把边定成 0/1，而是先记住**边之间的关系**。
// 设某格还需要 r 条边、有 m 条待定边：
//   r=1, m=2                  => 两条恰好用一条 => 记 OPPOSITE
//   r=1, m=3 且某对 EQUAL     => 那对要么都用(2>1)要么都不用 => **两条都禁用**，第三条必用
//   r=1, m=3 且某对 OPPOSITE  => 那对已占掉唯一名额 => **第三条禁用**
//   r=2, m=3 且某对 EQUAL     => 都不用则只剩 1 条不够 => 那对必用，**第三条禁用**
//   r=2, m=3 且某对 OPPOSITE  => 那对贡献恰好 1 条 => 第三条必用
//   r=2, m=4 且某对 OPPOSITE  => 该对贡献 1 => 另两条也恰好贡献 1 => 记另两条 OPPOSITE
// 关系靠**带奇偶的并查集**维护；一条边同属两个格，所以关系会在格与格之间传播。
// EQUIV=0 可关掉做对照。
//
// 以下是 v23 的说明。
...
// ===== 等价推理层：边之间的 EQUAL(0) / OPPOSITE(1) 关系 =====
// 无向边的规范编号：向右 = c*2，向下 = c*2+1
static inline int eid(int c, int d) {
    if (d >= 2) return c * 2 + (d - 2);
    int n = c + delta[d];
    return n * 2 + ((d ^ 2) - 2);
}

static int *edsu; static unsigned char *epar;     // epar[x] = x 相对于父亲的奇偶
static long long equiv_bans = 0, equiv_uses = 0;
static long long eq_unions = 0, eq_c12 = 0, eq_c13 = 0, eq_c23 = 0, eq_c24 = 0, eq_skip_end = 0;
static int use_equiv = 1;

static int efind(int x, int *par) {
    int p = 0, r = x;
    while (edsu[r] != r) { p ^= epar[r]; r = edsu[r]; }
    // 路径压缩，同时把奇偶折叠到根
    int cur = x, cp = p;
    while (edsu[cur] != cur) {
        int nx = edsu[cur], np = cp ^ epar[cur];
        edsu[cur] = r; epar[cur] = (unsigned char)cp;
        cur = nx; cp = np;
    }
    *par = p;
    return r;
...
// 断言 rel(a,b) = v（0 相同 / 1 相反）。返回 0 表示与已有关系矛盾。
static int eunion(int a, int b, int v) {
    int pa, pb, ra = efind(a, &pa), rb = efind(b, &pb);
    if (ra == rb) return (pa ^ pb) == v;
    edsu[ra] = rb; epar[ra] = (unsigned char)(pa ^ pb ^ v);
    return 1;
}

// 已知关系则返回 0/1，未知返回 -1
static int erel(int a, int b) {
    int pa, pb, ra = efind(a, &pa), rb = efind(b, &pb);
    return ra == rb ? (pa ^ pb) : -1;
}

static int do_equiv(void) {
    if (!use_equiv) return !prop_bad;
    for (int c = 0; c < N; ++c) if (g0[c]) { }          // 占位，保持结构清晰
    for (int round = 0; round < 6 && !prop_bad; ++round) {
        int changed = 0;
        for (int c = 0; c < N && !prop_bad; ++c) {
            if (!g0[c]) continue;
            int und[4], nu = 0, nfix = 0;
            for (int d = 0; d < 4; ++d) {
                if (!g0[c + delta[d]]) continue;
                if (estate[c * 4 + d] == 1) ++nfix;
                else if (estate[c * 4 + d] == 0) und[nu++] = d;
            }
            // 端点只用 1 条边，是谁又不知道，所以这里只处理**已知不可能是端点**的格：
            // 传播层已经用「终点颜色」把一半的格子排除了，其余格只能保守地按 r=2 处理。
            if (col[c] == prop_endcol || c == prop_start) ++eq_skip_end;
            if (col[c] != prop_endcol && c != prop_start) {
                int r = 2 - nfix;
                if (r < 0) { prop_bad = 1; break; }
                if (nu == 2 && r == 1) {
                    ++eq_c12; ++eq_unions;
                    if (!eunion(eid(c, und[0]), eid(c, und[1]), 1)) { prop_bad = 1; break; }
                } else if (nu == 3 && r == 1) {
                    ++eq_c13;
                    for (int i = 0; i < 3 && !prop_bad; ++i)
                        for (int j = i + 1; j < 3; ++j) {
                            int rel = erel(eid(c, und[i]), eid(c, und[j]));
                            int k = 3 - i - j;             // 剩下那条的下标
                            if (rel == 0) {                // 这对同进同出 => 都不能用
                                set_edge(c, und[i], 2); set_edge(c, und[j], 2);
                                set_edge(c, und[k], 1);
                                equiv_bans += 2; equiv_uses++; changed = 1; goto next_cell;
                            } else if (rel == 1) {         // 这对恰好占掉唯一名额
                                set_edge(c, und[k], 2); ++equiv_bans; changed = 1; goto next_cell;
                            }
                        }
                } else if (nu == 3 && r == 2) {
                    ++eq_c23;
                    for (int i = 0; i < 3 && !prop_bad; ++i)
                        for (int j = i + 1; j < 3; ++j) {
                            int rel = erel(eid(c, und[i]), eid(c, und[j]));
                            int k = 3 - i - j;
                            if (rel == 0) {                // 都不用则只剩 1 条，不够 2
                                set_edge(c, und[i], 1); set_edge(c, und[j], 1);
                                set_edge(c, und[k], 2);
                                ++equiv_bans; equiv_uses += 2; changed = 1; goto next_cell;
                            } else if (rel == 1) {         // 这对恰好贡献 1 条
                                set_edge(c, und[k], 1); ++equiv_uses; changed = 1; goto next_cell;
                            }
                        }
                } else if (nu == 4 && r == 2) {
                    ++eq_c24;
                    for (int i = 0; i < 4 && !prop_bad; ++i)
                        for (int j = i + 1; j < 4; ++j) {
                            if (erel(eid(c, und[i]), eid(c, und[j])) != 1) continue;
                            int a = -1, b = -1;
                            for (int t = 0; t < 4; ++t) if (t != i && t != j) { if (a < 0) a = t; else b = t; }
                            if (!eunion(eid(c, und[a]), eid(c, und[b]), 1)) { prop_bad = 1; }
                            goto next_cell;
                        }
                }
            }
...
static int propagate_strong(int s) {
    ...
    for (int i = 0; i < 2 * N; ++i) { edsu[i] = i; epar[i] = 0; }   // 关系是按起点算的，每次重置
    if (!do_equiv()) return 0;
    if (!do_probing(1)) return 0;                       // 新禁用边喂回单边 probing 再滚一轮
    return orient_chains(s);
}
...
int main(int argc, char **argv) {
    ...
    edsu = malloc(sizeof(int) * (size_t)(2 * N)); epar = malloc((size_t)(2 * N));
    if (getenv("EQUIV")) use_equiv = atoi(getenv("EQUIV"));
    ...
    if (argc > 3 && !strcmp(argv[2], "--verify")) {
        ...
        {   long long ne = 0, nu = 0, nb = 0;
            ...
            printf("边 %lld：必用 %lld，禁用 %lld，判定率 %.1f%%（等价推理贡献：禁用 %lld / 必用 %lld）\n",
                   ne, nu, nb, 100.0 * (nu + nb) / (ne ? ne : 1), equiv_bans, equiv_uses);
            printf("等价层：建立关系 %lld 次；可用格分布 (nu=2,r=1)=%lld (nu=3,r=1)=%lld "
                   "(nu=3,r=2)=%lld (nu=4,r=2)=%lld；因端点色跳过 %lld\n",
                   eq_unions, eq_c12, eq_c13, eq_c23, eq_c24, eq_skip_end);
        }
