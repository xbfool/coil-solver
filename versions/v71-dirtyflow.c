// Coil solver — v66: 修 v65 的 mine_total=0（动态取号把存活率门废掉了）
//
// 以下是 v65 的说明。
...
// 分层计时：漏斗的钱到底花在哪一层（PROPSTAT=1 打印）
static double t_prop, t_flow, t_chain, t_probe;
static double t_fbuild, t_fmax, t_fscc;   // do_flow 内部三段：建网络 / Dinic增广 / 残量图SCC
static long long n_prop, n_flow, n_chain, n_probe;
// 分层计时用**进程 CPU 时间**而不是墙钟：机器上还有别的活，墙钟会随争抢漂
// （实测同一配置两次量到 12.11s 和 21.39s）。CPU 时间量的才是算法开销本身。
static inline double now_s(void) {
    struct timespec ts; clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts);
    return ts.tv_sec + ts.tv_nsec * 1e-9;
}
#define LAYER(acc, cnt, expr) ({ double _a = now_s(); int _r = (expr); acc += now_s() - _a; ++cnt; _r; })


// ===== v68：全盘传播不动点按「端点颜色」缓存 =====
//
// pprocess 对 s 以外的每一格，规则只依赖 prop_endcol —— 而 prop_endcol 由 col[s] 决定，只有 2 种取值。
// 也就是说「推全盘 33486 格 + 跑到不动点」这件事，**16743 个同色起点算的是同一个东西**，
// 却各自从头算了一遍：O(n) 的活干了 n 遍 = O(n²)。
//
// 改法：每种颜色只算一次基座（不假定任何格是起点，所以推论对该色所有起点都成立），
// 之后每个起点只 memcpy 基座 + 推入 s 这一格，让级联在局部收敛。
static unsigned char *base_est[2];
static int *base_dsu[2];
static int *base_pend[2];      // 基座里「只剩 1 条可用边」的格子：起点未知时不能断言它是终点，留到起点已知再查
static unsigned char *base_mark[2];
static int base_npend[2], base_fend[2], base_bad[2], base_ready[2];
static int prop_nostart;       // 建基座时置 1：不假定任何格是起点
static int use_basecache = 1;
static int last_base_ok;           // 本次 propagate 是否命中了传播基座（决定 do_flow 用哪套流基座）

// v71：dirty 格清单 —— 记下 estate 相对"载入的那份快照"变了的格子，热启动只重算这些格关联的弧。
// 关键简化：set_edge(c,d,·) 会同时标记 c 和 n，而相邻两格必然异色，
// **所以格-格弧的 X 侧端点一定在 dirty 集里** => 只处理 dirty 集里的 X 色格就覆盖了全部格-格弧。
static int *dirty; static unsigned char *dmark; static int ndirty;
static int use_dirtyflow = 1;
static int dirty_valid;      // dirty 清单是否对得上当前 estate；二三层会绕过 propagate 直接 memcpy 恢复 estate，那时候必须退回整遍扫描
static inline void dmark_cell(int c) { if (!dmark[c]) { dmark[c] = 1; dirty[ndirty++] = c; } }
static inline void dirty_reset(void) {
    for (int i = 0; i < ndirty; ++i) dmark[dirty[i]] = 0;   // 只清列过的，别扫全盘
    ndirty = 0; dirty_valid = 1;
}
static long long bc_try, bc_hit;   // 基座命中率：没碰过 s 的边才敢用
...
static void set_edge(int c, int d, int v) {
    ...
    *a = *b = (unsigned char)v;
    ppush(c); ppush(n);
    dmark_cell(c); dmark_cell(n);
}

static long long pp_calls;          // 传播工作量计数：判断基座缓存有没有真省下活
static void pprocess(int c) {
    ...
    if (c != prop_start && used + avail == 1 && hi == 2) {   // 最多挂 1 条边 => 只能是端点
        if (prop_nostart) {          // 建基座时起点未知：这格可能是**起点**而不是终点，不能断言颜色。记下来，等起点定了再查
            if (!base_mark[prop_endcol][c]) {
                base_mark[prop_endcol][c] = 1;
                base_pend[prop_endcol][base_npend[prop_endcol]++] = c;
            }
            return;
        }
...
// 建某个端点颜色的基座：不假定任何格是起点，所以推出来的边对该色的**所有**起点都成立。
static void build_base(int ec) {
    if (base_ready[ec]) return;
    if (!base_est[ec]) {
        base_est[ec]  = malloc((size_t)N * 4);
        base_dsu[ec]  = malloc((size_t)N * sizeof(int));
        base_pend[ec] = malloc((size_t)N * sizeof(int));
        base_mark[ec] = calloc((size_t)N, 1);
    }
    dsu0_valid = 0;
    memcpy(estate, g_estate, (size_t)N * 4);
    for (int c = 0; c < N; ++c) dsu[c] = c;
    memset(inq, 0, (size_t)N);
    memset(base_mark[ec], 0, (size_t)N);
    qhead = qtail = 0;
    base_npend[ec] = 0;
    prop_bad = 0; prop_forced_end = -1; prop_start = -1; prop_endcol = ec; prop_nostart = 1;
    for (int c = 0; c < N; ++c) if (g0[c]) ppush(c);
    prun_queue();
    prop_nostart = 0;
    base_bad[ec] = prop_bad;
    base_fend[ec] = prop_forced_end;
    memcpy(base_est[ec], estate, (size_t)N * 4);
    memcpy(base_dsu[ec], dsu, (size_t)N * sizeof(int));
    base_ready[ec] = 1;
}

static int propagate(int s) {
    dsu0_valid = 0;
    int ec = (total_free & 1) ? col[s] : (col[s] ^ 1);

    // ⚠ 基座对「col != endcol」的格子断言 lo=hi=2（绝非端点）——但**起点自己就是这个颜色**，
    //   对 s 而言这是假前提，而传播单调、事后撤不回来（v68 初版 L351 因此把真起点误杀，输出空）。
    //   这个假前提唯一能产生的推论，是在 s 自己那 4 条边上赋值（used==hi 禁边 / used+avail==lo 定边，
    //   两条规则都只动格 c 的边）。所以：**基座没碰过 s 的边 => 基座对 s 可靠**；碰过就退回全盘传播。
    //   （s 的边也可能是被邻居的合法规则定的，那样会白退一次——保守，但可靠。）
    int base_ok = 0;
    if (use_basecache && !getenv("FORCEEDGES")) {
        build_base(ec);
        if (!base_bad[ec]) {                        // base_bad 也可能来自假前提，不能据此判该色全灭
            base_ok = 1;
            for (int d = 0; d < 4; ++d)
                if (g0[s + delta[d]] && base_est[ec][s * 4 + d] != g_estate[s * 4 + d]) { base_ok = 0; break; }
        }
        ++bc_try; bc_hit += base_ok;
    }
    // do_flow 靠这个选用哪套流基座：estate 从哪份快照长出来，弧集就必须取那一份的超集。
    last_base_ok = base_ok;
    if (base_ok) {
        memcpy(estate, base_est[ec], (size_t)N * 4);
        memcpy(dsu, base_dsu[ec], (size_t)N * sizeof(int));
        dirty_reset(); dmark_cell(s);
        memset(inq, 0, (size_t)N);
        qhead = qtail = 0;
        prop_bad = 0; prop_forced_end = base_fend[ec]; prop_start = s; prop_endcol = ec;
        ppush(s);                                                    // 只推 s 这一格，级联自己扩散
        for (int d = 0; d < 4; ++d) if (g0[s + delta[d]]) ppush(s + delta[d]);
        for (int i = 0; i < base_npend[ec]; ++i) ppush(base_pend[ec][i]);  // 补查基座里跳过的端点规则
        prun_queue();
        goto after_full;
    }
...
static int use_warmflow = 1;               // v69：max-flow 热启动（见 do_flow 上方长注释）
struct fbase {
    int ready, nodes, cnt;
    int *head, *next, *to, *capf;          // capf = 基座跑完 max-flow 后的残量
    signed char *kind; int *acell, *adir;  // 弧元信息（只对正向弧有效）
    int *fmap0;                            // 基座的 fmap（弧集固定，每起点只需把已定边改回 -1）
    long long flow;
    // v71：dirty 增量需要的东西 —— 每格的源/汇弧、每格的哑点弧、以及基座的 needL/needR
    int *cell_arc, *cell_dum;
    long long needL0, needR0;
};
static struct fbase fb[2][2];   // [起点色][slot] slot0=从 g_estate 建(对所有起点可靠) slot1=从 base_est 建(只给 base_ok 的起点)
...
static int global_fixpoint(void) {
    ...
    base_ready[0] = base_ready[1] = 0;
    fb[0][0].ready = fb[0][1].ready = fb[1][0].ready = fb[1][1].ready = 0;          // 基座是从 g_estate 长出来的，g_estate 一变就得作废
...
// ===== v69：max-flow 热启动 =====
//
// 分层计时显示 flow 占漏斗 92~94%（L664 第一层 66s 里 61s 是它），而其中 ~65% 是 Dinic 从零增广：
// 每个起点都要把 needL ≈ 16743 个单位重新推一遍，可这些流在起点之间几乎完全一样。
//
// 结构上极其有利：
//   · 图是 4 层 DAG（SRC → X色格 → ~X色格 → SNK，哑点插在其中一层），**增广路长度恒为 3**，
//     所以退流不用搜路径，改 3 条弧就行；
//   · 每起点相对基座**容量只降不升**（estate 只会更定 => nfix 只增 => rb 只减；格 s 的 b 从 2 变 1），
//     只降不升是增量最大流最好办的情形：只需退掉超额的流，不需要处理新增容量。
//
// ⚠ 基座网络必须从 **g_estate** 建，不能从 v68 的传播基座 base_est 建 ——
//   base_est 在格 s 上带假前提，可能"定"了 s 的边，弧集就不再是每起点弧集的超集了。
//   g_estate 是全局相结论，对所有起点都可靠，弧集必然是超集。
#define AK_SRC2C 0    // FSRC -> cell        (col==X)
#define AK_C2SNK 1    // cell -> FSNK        (col!=X)
#define AK_C2C   2    // cell -> cell        (待定边)
#define AK_SRC2D 3    // FSRC -> FDUM
#define AK_D2C   4    // FDUM -> cell
#define AK_C2D   5    // cell -> FDUM
...
static void fcancel_unit(int e) {
    fcap[e] += 1; fcap[e ^ 1] -= 1;                       // 先把这条弧上的 1 个单位退掉
    int u = fto[e ^ 1], v = fto[e];
    while (u != FSRC) {                                   // 往回退到源
        int found = -1;
        for (int a = fhead[u]; a != -1; a = fnext[a])
            if ((a & 1) && fcap[a] > 0) { found = a; break; }   // 奇数=反向弧，fcap>0 表示正向弧 a^1 载流
        if (found < 0) { ++warm_broken; break; }   // 流守恒保证这条不会走到
        fcap[found] -= 1; fcap[found ^ 1] += 1;
        u = fto[found];
    }
    while (v != FSNK) {                                   // 往前退到汇
        int found = -1;
        for (int a = fhead[v]; a != -1; a = fnext[a])
            if (!(a & 1) && fcap[a ^ 1] > 0) { found = a; break; } // 偶数=正向弧，其反向弧 fcap>0 表示载流
        if (found < 0) { ++warm_broken; break; }
        fcap[found] += 1; fcap[found ^ 1] -= 1;
        v = fto[found];
    }
    ++warm_cancel;
...
static signed char *bkind; static int *bcell, *bdir;   // 建基座时临时记弧元信息
static void fadd_k(int u, int v, int cap, int k, int c, int d) {
    bkind[fcnt] = (signed char)k; bcell[fcnt] = c; bdir[fcnt] = d;
    fadd(u, v, cap);
}

// 建某个起点色 X 的基座流网络：弧集取**超集**（用 g_estate，b 一律取 2，哑点弧全带上），
// 这样每个起点的网络都只是"把某些弧的容量调低"，结构完全一致，fcap 一次 memcpy 就能复用。
static void build_flow_base(int X, int slot, const unsigned char *est) {
    struct fbase *B = &fb[X][slot];
    if (B->ready) return;
    if (!bkind) { bkind = malloc((size_t)N * 24); bcell = malloc((size_t)N * 24 * sizeof(int)); bdir = malloc((size_t)N * 24 * sizeof(int)); }

    fnodes = N + 3;
    for (int i = 0; i < fnodes; ++i) fhead[i] = -1;
    fcnt = 0;
    for (int i = 0; i < N * 4; ++i) fmap[i] = -1;

    if (!B->cell_arc) { B->cell_arc = malloc((size_t)N * sizeof(int)); B->cell_dum = malloc((size_t)N * sizeof(int)); }
    for (int c = 0; c < N; ++c) { B->cell_arc[c] = -1; B->cell_dum[c] = -1; }
    long long nL = 0, nR = 0;

    for (int c = 0; c < N; ++c) {
        if (!g0[c]) continue;
        int nfix = 0;
        for (int d = 0; d < 4; ++d) if (g0[c + delta[d]] && est[c * 4 + d] == 1) ++nfix;
        int rb = 2 - nfix; if (rb < 0) rb = 0;
        B->cell_arc[c] = fcnt;
        if (col[c] == X) { fadd_k(FSRC, FCELL(c), rb, AK_SRC2C, c, 0); nL += rb; }
        else             { fadd_k(FCELL(c), FSNK, rb, AK_C2SNK, c, 0); nR += rb; }
    }
    if (prop_endcol != X) {
        fadd_k(FSRC, FDUM, 1, AK_SRC2D, 0, 0); nL += 1;
        for (int c = 0; c < N; ++c) if (g0[c] && col[c] != X) fadd_k(FDUM, FCELL(c), 1, AK_D2C, c, 0);
    } else {
        fadd_k(FDUM, FSNK, 1, AK_D2SNK, 0, 0); nR += 1;
        for (int c = 0; c < N; ++c) if (g0[c] && col[c] == X) { B->cell_dum[c] = fcnt; fadd_k(FCELL(c), FDUM, 1, AK_C2D, c, 0); }
    }
    B->needL0 = nL; B->needR0 = nR;
    for (int c = 0; c < N; ++c) {
        if (!g0[c] || col[c] != X) continue;
        for (int d = 0; d < 4; ++d) {
            int n = c + delta[d];
            if (!g0[n] || est[c * 4 + d] != 0) continue;
            fmap[c * 4 + d] = fcnt;
            fadd_k(FCELL(c), FCELL(n), 1, AK_C2C, c, d);
        }
    }
    ...
    B->nodes = fnodes; B->cnt = fcnt; B->flow = flow;
    B->head = malloc((size_t)fnodes * sizeof(int));
    B->next = malloc((size_t)fcnt * sizeof(int));
    B->to   = malloc((size_t)fcnt * sizeof(int));
    B->capf = malloc((size_t)fcnt * sizeof(int));
    B->kind = malloc((size_t)fcnt);
    B->acell = malloc((size_t)fcnt * sizeof(int));
    B->adir  = malloc((size_t)fcnt * sizeof(int));
    B->fmap0 = malloc((size_t)N * 4 * sizeof(int));
    memcpy(B->head, fhead, (size_t)fnodes * sizeof(int));
    memcpy(B->next, fnext, (size_t)fcnt * sizeof(int));
    memcpy(B->to,   fto,   (size_t)fcnt * sizeof(int));
    memcpy(B->capf, fcap,  (size_t)fcnt * sizeof(int));
    memcpy(B->kind, bkind, (size_t)fcnt);
    memcpy(B->acell, bcell, (size_t)fcnt * sizeof(int));
    memcpy(B->adir,  bdir,  (size_t)fcnt * sizeof(int));
    memcpy(B->fmap0, fmap, (size_t)N * 4 * sizeof(int));
    B->ready = 1;
    if (getenv("PROPSTAT"))
        fprintf(stderr, "  flow基座 X=%d slot=%d: %d 弧, 基座流 %lld\n", X, slot, fcnt, flow);
}

static int do_flow_warm(int s, int X, int slot) {
    struct fbase *B = &fb[X][slot];
    fnodes = B->nodes; fcnt = B->cnt;
    memcpy(fhead, B->head, (size_t)fnodes * sizeof(int));
    memcpy(fnext, B->next, (size_t)fcnt * sizeof(int));
    memcpy(fto,   B->to,   (size_t)fcnt * sizeof(int));
    memcpy(fcap,  B->capf, (size_t)fcnt * sizeof(int));
    memcpy(fmap,  B->fmap0, (size_t)N * 4 * sizeof(int));
    long long flow = B->flow, needL = 0, needR = 0;

    if (use_dirtyflow && dirty_valid) {
        // 只重算 dirty 格关联的弧。memcpy 已把其余所有弧复位成基座容量，那些格的 estate
        // 相对基座没变，容量本来就该是基座值 —— 所以整遍 O(E) 可以跳过。
        needL = B->needL0; needR = B->needR0;
        for (int i = 0; i < ndirty; ++i) {
            int c = dirty[i];
            if (!g0[c]) continue;
            int e = B->cell_arc[c];
            if (e >= 0) {                                    // 源/汇弧：rb = b - nfix 变了
                int b = (c == s) ? 1 : 2, nfix = 0;
                for (int d = 0; d < 4; ++d) if (g0[c + delta[d]] && estate[c * 4 + d] == 1) ++nfix;
                int nc = b - nfix;
                if (nc < 0) { prop_bad = 1; return 0; }
                int oc = fcap[e] + fcap[e ^ 1];              // 基座容量 = 残量 + 已用流量
                if (col[c] == X) needL += nc - oc; else needR += nc - oc;
                while (fcap[e ^ 1] > nc) { fcancel_unit(e); --flow; }
                fcap[e] = nc - fcap[e ^ 1];
            }
            if ((e = B->cell_dum[c]) >= 0 && c == s) {       // 哑点在汇侧：起点自己不能当终点
                while (fcap[e ^ 1] > 0) { fcancel_unit(e); --flow; }
                fcap[e] = 0;
            }
            if (col[c] == X) {                               // 格-格弧只挂在 X 侧；异色那端由 set_edge 一并标脏，覆盖完整
                for (int d = 0; d < 4; ++d) {
                    int a = B->fmap0[c * 4 + d];
                    if (a < 0) continue;
                    if (estate[c * 4 + d] == 0) continue;    // 还没定，容量仍是基座的 1
                    fmap[c * 4 + d] = -1;                    // 已定：下游 subtour/SCC 才不会误当载流弧
                    while (fcap[a ^ 1] > 0) { fcancel_unit(a); --flow; }
                    fcap[a] = 0;
                }
            }
        }
    ...
    for (int e = 0; e < fcnt; e += 2) {          // 一遍 O(E)：算新容量，超额就退流
        int c = B->acell[e], nc;
        switch (B->kind[e]) {
        case AK_SRC2C: case AK_C2SNK: {
            int b = (c == s) ? 1 : 2, nfix = 0;
            for (int d = 0; d < 4; ++d) if (g0[c + delta[d]] && estate[c * 4 + d] == 1) ++nfix;
            nc = b - nfix;
            if (nc < 0) { prop_bad = 1; return 0; }
            if (B->kind[e] == AK_SRC2C) needL += nc; else needR += nc;
            break; }
        case AK_C2C: {
            int d = B->adir[e];
            nc = (estate[c * 4 + d] == 0);
            if (!nc) fmap[c * 4 + d] = -1;       // 已定的边：把 fmap 改回 -1，下游 subtour/SCC 才不会误当载流弧
            break; }
        case AK_C2D:  nc = (c != s); break;      // 哑点在汇侧时，起点自己不能当终点
        case AK_SRC2D: nc = 1; needL += 1; break;
        case AK_D2SNK: nc = 1; needR += 1; break;
        default:       nc = 1; break;            // AK_D2C
        }
        while (fcap[e ^ 1] > nc) { fcancel_unit(e); --flow; }
        fcap[e] = nc - fcap[e ^ 1];
    }
caps_done:
    ...
    long long before = flow;
    while (fbfs()) {
        for (int i = 0; i < fnodes; ++i) fiter[i] = fhead[i];
        int f;
        while ((f = fdfs(FSRC, 1 << 30)) > 0) flow += f;
    }
    warm_aug += flow - before;
...
static int do_flow(int s) {
    ...
    int X = col[s];
    if (use_warmflow) {
        int slot = last_base_ok ? 1 : 0;
        build_flow_base(X, slot, slot ? base_est[prop_endcol] : g_estate);
        int r = do_flow_warm(s, X, slot);
        t_fbuild += now_s() - tf0;
        if (!r) return 0;
        goto after_maxflow;
    }
...
after_maxflow:;
    ...
    for (int e = fhead[FDUM]; e != -1; e = fnext[e]) {
        ...
        if (fcap[e] + fcap[e ^ 1] == 0) continue;   // 热启动下容量被调成 0 的弧＝原版里根本不存在的弧
...
static int propagate_strong(int s) {
    if (!LAYER(t_prop, n_prop, propagate(s))) return 0;   // 第 1 层：最便宜
    ...
    if (!LAYER(t_flow, n_flow, do_flow(s))) return 0;                   // 第 2 层：一次 max-flow
    if (use_chain && !LAYER(t_chain, n_chain, orient_chains(s))) return 0; // 第 3 层：0.3% 的代价，98% 的证伪
    if (getenv("XCHAIN") && !xchain_check(s)) return 0;   // 第 3.5 层：跨链时序偏序（v42 复活）
    if (probe_rounds <= 0) return 1;             // 默认到此为止
    if (!LAYER(t_probe, n_probe, do_probing(probe_rounds))) return 0;
    if (!LAYER(t_flow, n_flow, do_flow(s))) return 0;
    if (!LAYER(t_probe, n_probe, do_probing(1))) return 0;
    return orient_chains(s);
}
...
int main(int argc, char **argv) {
    ...
    inq = malloc((size_t)N);
    dirty = malloc((size_t)N * sizeof(int)); dmark = calloc((size_t)N, 1); ndirty = 0;
    ...
    if (getenv("BASECACHE")) use_basecache = atoi(getenv("BASECACHE"));
    if (getenv("WARMFLOW")) use_warmflow = atoi(getenv("WARMFLOW"));
    if (getenv("DIRTYFLOW")) use_dirtyflow = atoi(getenv("DIRTYFLOW"));
  ...
  child_work:;
    ...
    int mine_total = 0;          // 本片实际领到并处理的起点数（动态取号下只能边跑边数）
    ...
    for (int ii = 0; ; ++ii) {
        ...
        int s = starts[i];
        ++mine_total;            // ⚠ 必须在这里数：动态取号下事先算不出「我这片有多少个」，
                                 //   而下游的存活率门（第二层开关 / SURVDEEP 升深传播）全靠它。
                                 //   v65 漏了这一步 => mine_total=0 => 存活率算成 0% =>
                                 //   第二层整个跳过、深传播不升档 => 漏斗看着快了一倍，
                                 //   实则变弱：每片幸存者从 59 涨到 564~768，第三层根本扫不完。
    ...
    if (getenv("PROPSTAT")) {
        fprintf(stderr, "PROPSTAT shard %d starts=%d keep=%d work=%lld basehit=%lld/%lld | prop %.2fs(%lld) flow %.2fs(%lld) chain %.2fs(%lld) probe %.2fs(%lld)\n",
                shard, mine_total, keep, pp_calls, bc_hit, bc_try,
                t_prop, n_prop, t_flow, n_flow, t_chain, n_chain, t_probe, n_probe);
        fprintf(stderr, "  flow-inner shard %d: build %.2fs / augment %.2fs / residual-SCC %.2fs | flow证伪 %lld 退流 %lld 热增广 %lld\n",
                shard, t_fbuild, t_fmax, t_fscc, flow_refute, warm_cancel, warm_aug);
        if (warm_broken) fprintf(stderr, "  ⚠ shard %d: 退流走断 %lld 次 —— 流已非法，结果不可信\n", shard, warm_broken);
    }
    {
        ...
        if (prop_depth <= 2 && mine_total > 0 && keep * 100 >= mine_total * sdthr) {
            ...
            for (int i = 0; i < keep; ++i) {
                int s = starts[i];
                memcpy(estate, est_save[i], (size_t)N * 4); dsu0_valid = 0; dirty_valid = 0;
    ...
    for (int i0 = 0; i0 < keep; ++i0) {
        ...
        memcpy(estate, est_save[i], (size_t)N * 4); dsu0_valid = 0; dirty_valid = 0;   // 复用第一层缓存好的 estate，不重算
        memcpy(tcand, tc_save[i], (size_t)N);
        tcand_for = s;
        mark(s);
