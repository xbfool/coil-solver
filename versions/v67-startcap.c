// Coil solver — v67: 单起点上限（超时当「没证伪」放行）—— 治「一个超长任务拖住全局」
//
// 以下是 v66 的说明。
...
static long long cap_starts = 0, cap_fired = 0;
...
// ===== 单起点上限 =====
// 实测 L664：动态取号把漏斗匀成 136/149 秒，但**抓到怪物起点的那个核仍要在那一个起点上磨 579 秒**
// —— 动态取号能匀「多个任务」，匀不了「一个超长任务」。
// 对策：给单个起点的传播设墙钟上限，超了就中断，**当作「没证伪」放行**。
// 这个方向永远 sound：证伪失败 = 保留候选，少推导只会少剪、不会剪错；
// 代价仅是该起点进第三层多占一点时间。
static long long cap_ms = 0;          // 0 = 不限；STARTCAP 毫秒
static long long cap_t0 = 0;
static int cap_hit = 0;
static long long cap_tick = 0;

static inline long long now_ms(void) {
    struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts);
    return (long long)ts.tv_sec * 1000 + ts.tv_nsec / 1000000;
}
static inline int cap_over(void) {                 // 每 1024 次才真去读时钟
    if (!cap_ms || cap_hit) return cap_hit;
    if ((++cap_tick & 1023)) return 0;
    if (now_ms() - cap_t0 > cap_ms) cap_hit = 1;
    return cap_hit;
}

static void prun_queue(void) {
    while (qhead != qtail && !prop_bad) {
        if (cap_over()) return;                    // 超时：中断传播，estate 里已推的部分仍然有效
        int c = pq[qhead++];
...
static int propagate_strong(int s) {
    ...
    if (cap_over()) return 1;                    // 超时 => 当作没证伪（保留候选，sound）
...
int main(int argc, char **argv) {
    ...
    cap_ms = getenv("STARTCAP") ? atoll(getenv("STARTCAP")) : 20000;   // 默认单起点 20 秒
  ...
  child_work:;
    ...
    for (int ii = 0; ; ++ii) {
        ...
        int s = starts[i];
        ++mine_total;
        if (cap_ms) { cap_t0 = now_ms(); cap_hit = 0; cap_tick = 0; ++cap_starts; }            // ⚠ 必须在这里数：动态取号下事先算不出「我这片有多少个」，
