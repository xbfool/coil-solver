...
static int adapt_on = 0;
static int wp_period[2] = {128, 128};   // [0]=region_parity [1]=propagate_dyn
...
static int dfs(int p, int remaining, int depth) {
    ...
    if (adapt_on && remaining > 0) {
        if ((nodes % wp_period[0]) == 0) {                       // 武器0：region_parity（767型主力）
            ++wp_try[0];
            if (!rp_dyn_ok(p, remaining)) { ++wp_kill[0]; wp_period[0] -= wp_period[0]/4 + 1; if (wp_period[0] < WP_MIN) wp_period[0] = WP_MIN; bj_seeking = 1; return 0; }
            wp_period[0] += wp_period[0]/8 + 1; if (wp_period[0] > WP_MAX) wp_period[0] = WP_MAX;
        }
        if ((nodes % wp_period[1]) == 0) {                       // 武器1：propagate_dyn 强制边（747型主力）
            int r0 = -1; for (int d = 0; d < 4; ++d) if (g[p + delta[d]]) { r0 = p + delta[d]; break; }
            if (r0 >= 0) { ++wp_try[1];
                if (!propagate_dyn(p, remaining, r0, delta[0], -1)) { ++wp_kill[1]; wp_period[1] -= wp_period[1]/4 + 1; if (wp_period[1] < WP_MIN) wp_period[1] = WP_MIN; bj_seeking = 1; return 0; }
                wp_period[1] += wp_period[1]/8 + 1; if (wp_period[1] > WP_MAX) wp_period[1] = WP_MAX;
            }
        }
    }
...
int main(int argc, char **argv) {
  ...
  child_work:;
    ...
    restart_on = getenv("RESTART") ? atoi(getenv("RESTART")) : 0;
    restart_k = getenv("RESTARTK") ? atoi(getenv("RESTARTK")) : 24;
