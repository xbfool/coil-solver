// Coil solver — v57: 有向进入方向 probing（Tron 路数：两案分解假设传播，全局禁用表进 estate_ok）
//
// 以下是 v53 的说明。
...
static unsigned char *dban;   // v57：有向进入禁用表 dban[c*4+d]=1 表示「从 d 侧进入 c」不可能
static void direntry_probe(void);
...
static int global_fixpoint(void) {
    ...
    if (!getenv("NODBAN")) direntry_probe();
...
static inline int estate_ok(int p, int d, int dd, int len, int c) {
    int od = d ^ 2;
    for (int k = 0, q = p + dd; ; q += dd) {                  // v57：run 上每一格都是「从 d 侧进入」
        if (dban[q * 4 + d]) return 0;
        if (q == c) break;
    }
...
static void pprocess2(int c) {
    ...
    else if (p2_endcol == -2)   { lo = 1; hi = 2; }     // v57 全局口径：终点色未知，全体保守 1~2
    ...
    if (p2_endcol != -2 && c != p2_start && used + avail == 1 && hi == 2) {
...
// v57：起点无关的假设传播——estate 底座 + 注入假设边 + 全量播种，返回 0 = 矛盾。
// p2_start 设成 -1（没有「当前头」的度=1 约束）；p2_endcol 设成 -2 时终点色未知：
// pprocess2 对非终点色格子要求度=2，终点色格子 1~2。全局口径下终点色由全局奇偶决定，
// 调用方传进来（total_free 偶数 => 两端异色，两色都可能是端点 => 传 -2 走保守 lo=1）。
static int hypo_bad;
static int hypo_propagate(int endcol_mode, int nhyp, const int *hyp_edge, const int *hyp_val) {
    if (!dsu0_valid) build_dsu0();
    memcpy(estate2, estate, (size_t)N * 4);
    memcpy(dsu2, dsu0, sizeof(int) * (size_t)N);
    q2h = q2t = 0;
    p2_bad = 0; p2_end = -1; p2_start = -1;
    p2_endcol = endcol_mode;
    for (int i = 0; i < nhyp && !p2_bad; ++i) {
        int e = hyp_edge[i], c = e / 4, d = e % 4;
        set_edge2(c, d, hyp_val[i]);
    }
    if (!p2_bad) {
        for (int c = 0; c < N; ++c) if (g[c]) ppush2(c);
        while (q2h != q2t && !p2_bad) {
...
// 全局有向进入 probing：两案（穿过/停下）皆矛盾 => 禁用 (c,d)。O(8N) 次传播。
static void direntry_probe(void) {
    memcpy(g, g0, N);
    int he[8], hv[8], nban = 0, total = 0;
    for (int c = 0; c < N; ++c) {
        if (!g0[c]) continue;
        for (int d = 0; d < 4; ++d) {
            int dd = delta[d];
            if (!g0[c - dd]) continue;                    // 从 d 侧进入需要 c-dd 自由
            ++total;
            int e1 = (d + 1) & 3;
            // 案 A：穿过。前驱 c-dd、后继 c+dd。c+dd 不自由 => 案 A 直接不可能。
            int okA = 0;
            if (g0[c + dd]) {
                int n = 0;
                he[n] = (c - dd) * 4 + d; hv[n] = 1; ++n;   // {c-dd,c} 用（从 c-dd 看方向 d）
                he[n] = c * 4 + d;        hv[n] = 1; ++n;   // {c,c+dd} 用
                he[n] = c * 4 + e1;       hv[n] = 2; ++n;   // 垂直边禁（set_edge2 会同步对面）
                he[n] = c * 4 + ((d + 3) & 3); hv[n] = 2; ++n;
                okA = hypo_propagate(-2, n, he, hv);
            }
            // 案 B：停下。前驱 c-dd，{c,c+dd} 必不用（c+dd 自由才有这条边可禁）。
            int okB;
            {
                int n = 0;
                he[n] = (c - dd) * 4 + d; hv[n] = 1; ++n;
                if (g0[c + dd]) { he[n] = c * 4 + d; hv[n] = 2; ++n; }
                okB = hypo_propagate(-2, n, he, hv);
            }
            if (!okA && !okB) { dban[c * 4 + d] = 1; ++nban; }
        }
    }
    fprintf(stderr, "  有向进入 probing：禁用 %d / %d 个 (格,向)（%.1f%%）\n",
            nban, total, total ? 100.0 * nban / total : 0.0);
}
