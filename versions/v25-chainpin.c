...
static int orient_chains(int s) {
    ...
    if (use_pin) { memset(pin_in, -1, (size_t)N); pins_set = 0; }
    int nchain = 0;
        ...
        if (use_pin && (ok0 != ok1)) {
            if (ok0) {                                      // 沿 seqbuf 正序走
                for (int i = 1; i < k; ++i) {
                    ...
                    for (int d = 0; d < 4; ++d)
                        if (delta[d] == diff) { pin_in[seqbuf[i]] = (signed char)d; ++pins_set; break; }
                }
            } else {                                        // 沿 seqbuf 逆序走
                for (int i = k - 2; i >= 0; --i) {
                    int diff = seqbuf[i] - seqbuf[i + 1];
...
static inline int estate_ok(int p, int d, int dd, int len, int c) {
    int od = d ^ 2;
    if (use_pin) {                                            // 有向剪枝：进入方向必须对得上
        for (int k = 0, q = p + dd; k < len; ++k, q += dd)
            if (pin_in[q] >= 0 && pin_in[q] != d) { ++pin_hits; return 0; }
    }
