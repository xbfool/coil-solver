...
//
// 以下是 v21 的说明。
// Coil solver — v27: 在 in 方向层上跑不动点 —— 链定向当种子，互斥约束当传播规则
//
// 诊断说「规则够强，缺的是种子」。v26 的链定向恰好提供了种子（8~10% 的格子 in 单值），
// 而昨晚找到的那条**无条件**互斥约束正好是传播规则：
//     ¬( in(c)=d ∧ in(c+d)=−d )     —— 两格的入边都被别人占了，中间那条边没人用，时序上直接矛盾
// 已知 in(c)=d 就能消掉 c+d 的一个候选。而**每格的 in 候选数就是它的度数，度 2 的格子只有 2 个**
// （占 30%）—— 消掉一个就变单值，可以往下雪崩。
//
// 另外两条免费的耦合：
//   · 边 (c 的前驱侧) 被禁用 => in(c) 去掉那个方向
//   · in(c)=d 定死 => 边 (c−d, c) 必用 => out(c−d)=d，顺手补一个 pin_out
// 候选集空掉 => 这个起点被证伪（白捡一条起点剪枝）。
// PINPROP=0 可关掉做对照。
//
...
static unsigned char *incand;      // 每格还可能的进入方向集合（4 bit）
static int *pworkq; static unsigned char *pinwq;
static int use_pinprop = 1;
static long long pinprop_gain = 0;   // 统计：互斥传播比链定向多钉死多少格

// in 方向层的不动点：链定向的 pins 当种子，互斥约束当规则，往下雪崩。
// 返回 0 表示某格候选集空掉 => 矛盾 => 这个起点被证伪。
static int propagate_pins(int s) {
    int qh = 0, qt = 0;
    for (int c = 0; c < N; ++c) {
        pinwq[c] = 0;
        if (!g0[c] || c == s) { incand[c] = 0; continue; }
        unsigned char m = 0;
        for (int d = 0; d < 4; ++d) {
            int pre = c + delta[d ^ 2];                    // in(c)=d 的前驱在 c−d
            if (!g0[pre]) continue;
            if (estate[c * 4 + (d ^ 2)] == 2) continue;    // 通往前驱的边被禁用了
            m |= (unsigned char)(1 << d);
        }
        if (pin_in[c] >= 0) m &= (unsigned char)(1 << pin_in[c]);   // 链定向的种子
        incand[c] = m;
        if (!m) return 0;
        if (__builtin_popcount(m) == 1) { pinwq[c] = 1; pworkq[qt++] = c; }
    }
    while (qh < qt) {
        int c = pworkq[qh++];
        pinwq[c] = 0;
        if (!incand[c] || __builtin_popcount(incand[c]) != 1) continue;
        int d = __builtin_ctz(incand[c]);
        int m2 = c + delta[d];
        if (!g0[m2] || m2 == s) continue;
        unsigned char before = incand[m2];
        unsigned char after = (unsigned char)(before & ~(1 << (d ^ 2)));   // 互斥：消掉「背对背」
        if (after == before) continue;
        if (!after) return 0;                              // 候选空 => 起点证伪
        incand[m2] = after;
        if (__builtin_popcount(after) == 1 && !pinwq[m2] && qt < N) { pinwq[m2] = 1; pworkq[qt++] = m2; }
    }
    // 回写：新定死的方向补进 pin_in / pin_out
    for (int c = 0; c < N; ++c) {
        if (!g0[c] || c == s || !incand[c]) continue;
        if (__builtin_popcount(incand[c]) != 1) continue;
        int d = __builtin_ctz(incand[c]);
        if (pin_in[c] < 0) { pin_in[c] = (signed char)d; ++pinprop_gain; }
        int pre = c + delta[d ^ 2];                        // in(c)=d => out(前驱)=d
        if (g0[pre] && pin_out[pre] < 0) pin_out[pre] = (signed char)d;
    }
    return 1;
}
...
static int orient_chains(int s) {
    ...
    if (use_pin && use_pinprop) return propagate_pins(s);
    return 1;
}
...
int main(int argc, char **argv) {
    ...
    incand = malloc((size_t)N); pworkq = malloc(sizeof(int) * (size_t)(N + 4));
    pinwq = malloc((size_t)N);
    if (getenv("PINPROP")) use_pinprop = atoi(getenv("PINPROP"));
    ...
    if (argc > 3 && !strcmp(argv[2], "--verify")) {
        ...
        {   // 传播产出统计：判定了多少条边，其中多少条是桥规则贡献的
            ...
            long long npin = 0;
            for (int c = 0; c < N; ++c) if (g0[c] && pin_in[c] >= 0) ++npin;
            printf("钉死进入方向 %lld / %d 格 (%.1f%%)：链定向 %lld + 互斥传播 %lld\n",
                   npin, total_free, 100.0 * npin / total_free, npin - pinprop_gain, pinprop_gain);
        }
