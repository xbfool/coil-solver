...
static int global_fixpoint(void) {
    ...
    base_ready[0] = base_ready[1] = 0; fb[0].ready = fb[1].ready = 0;          // 基座是从 g_estate 长出来的，g_estate 一变就得作废
...
static void build_flow_base(int X) {
    struct fbase *B = &fb[X];
    if (B->ready) return;
    ...
    for (int c = 0; c < N; ++c) {
        if (!g0[c]) continue;
        int nfix = 0;
        for (int d = 0; d < 4; ++d) if (g0[c + delta[d]] && g_estate[c * 4 + d] == 1) ++nfix;
    ...
    for (int c = 0; c < N; ++c) {
        ...
        for (int d = 0; d < 4; ++d) {
            int n = c + delta[d];
            if (!g0[n] || g_estate[c * 4 + d] != 0) continue;
            fmap[c * 4 + d] = fcnt;
    ...
    B->ready = 1;
    if (getenv("PROPSTAT"))
        fprintf(stderr, "  flow基座 X=%d: %d 弧, 基座流 %lld\n", X, fcnt, flow);
}

static int do_flow_warm(int s, int X) {
    struct fbase *B = &fb[X];
