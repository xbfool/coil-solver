// Coil solver — v59: 路径持久化增量传播（trailed fixpoint，不从头重播全盘）。实验版，v58 不动。
//
// 以下是 v53 的说明。
...
static int *dsz2;                 // v59：可回滚并查集的按size合并
static int *e2tr; static int e2trn;          // estate2 写入trail（存索引，回滚置0）
...
static void set_edge2(int c, int d, int v) {
    ...
    if (v == 1) {
        ...
        if (dsz2[ra] > dsz2[rb]) { int t = ra; ra = rb; rb = t; }   // 小挂大
        if (trailing) d2tr[d2trn++] = ra;      // 记录被挂走的根
        dsu2[ra] = rb; dsz2[rb] += dsz2[ra];
    }
    *a = *b = (unsigned char)v;
    if (trailing) { e2tr[e2trn++] = (int)(a - estate2); e2tr[e2trn++] = (int)(b - estate2); }
    ppush2(c); ppush2(n);
}
...
// 回滚到 trail 标记：撤销 estate2 写入(置0) 与 dsu2 union(复位根+size)
static void trail_rollback(int em, int dm) {
    while (e2trn > em) estate2[e2tr[--e2trn]] = 0;
    while (d2trn > dm) { int ra = d2tr[--d2trn]; int rb = dsu2[ra]; dsz2[rb] -= dsz2[ra]; dsu2[ra] = ra; }
}

// 增量传播：在当前 estate2/dsu2 上只播 seeds[] 里的格子，跑不动点。返回 !p2_bad。
static int propagate_delta(int p, int remaining, int *seeds, int nseed) {
    q2h = q2t = 0; p2_bad = 0; p2_end = -1; p2_start = p;
    p2_endcol = col[p] ^ (remaining & 1);
    for (int i = 0; i < nseed; ++i) if (g[seeds[i]]) ppush2(seeds[i]);
    while (q2h != q2t && !p2_bad) { int c = pq2[q2h++]; if (q2h == N + 1) q2h = 0; inq2[c] = 0; pprocess2(c); }
...
static int propagate_dyn(int p, int remaining, int sfirst, int sdd, int slen) {
    ...
    if (seed_mode == 0) {                      // v52 语义：空底座全播（不依赖 estate/dsu0）
        ...
        g[p] = 1;
        for (int c = 0; c < N; ++c) if (g[c]) { dsu2[c] = c; dsz2[c] = 1; ppush2(c); }
    } else {
        ...
        for (int c9 = 0; c9 < N; ++c9) dsz2[c9] = 1;   // 按size基座（dsu0已含必用边并查，size重算）
...
int main(int argc, char **argv) {
    ...
    dsz2 = malloc(sizeof(int) * (size_t)N);
    e2tr = malloc(sizeof(int) * (size_t)N * 8); d2tr = malloc(sizeof(int) * (size_t)N); incseeds = malloc(sizeof(int) * (size_t)N * 4);
