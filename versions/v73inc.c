...
// v77 增量连通性全局
static int off8[8];
static int *uf_root, *uf_rank, *uf_tc, *uf_frame, *uf_cell, *uf_qseen;
static signed char *uf_tr;
static int uf_ttop = 0, uf_fn = 0, uf_qgen = 0, inc_conn = 0;
static long long rl_skipped = 0, rl_taken = 0;
...
static void emit(int s, const char *pathstr) {
    ...
    if (inc_conn) { fprintf(stderr, "INCSTAT skipped=%lld taken=%lld skippct=%.1f", rl_skipped, rl_taken, (rl_skipped+rl_taken)?100.0*rl_skipped/(rl_skipped+rl_taken):0.0); fputc(10, stderr); }
...
// ===== v77 增量连通性：障碍并查集（平面对偶）+ 回滚 =====
// 自由区(4连通)分裂 <=> 障碍(墙+已访问,8连通)闭合出包住自由格的环。
// 蛇滑行=给障碍加一条直线段；段是简单路径,只在它触及同一障碍分量>=2处才成环。
// 无环 => 自由区必然仍连通 => 免去 reach_local 泛洪(可证 sound)。有环 => 回退泛洪定夺。
static inline int uf_find(int x){ while(uf_root[x]!=x) x=uf_root[x]; return x; }
static inline void uf_link0(int a,int b){ int ra=uf_find(a),rb=uf_find(b); if(ra==rb)return; if(uf_rank[ra]<uf_rank[rb]){int t=ra;ra=rb;rb=t;} uf_root[rb]=ra; if(uf_rank[ra]==uf_rank[rb])uf_rank[ra]++; }
static inline void uf_link_tr(int a,int b){ int ra=uf_find(a),rb=uf_find(b); if(ra==rb)return; if(uf_rank[ra]<uf_rank[rb]){int t=ra;ra=rb;rb=t;} uf_tc[uf_ttop]=rb; uf_tr[uf_ttop]=(signed char)(uf_rank[ra]==uf_rank[rb]); uf_ttop++; uf_root[rb]=ra; if(uf_rank[ra]==uf_rank[rb])uf_rank[ra]++; }
static void uf_add(int c){ uf_frame[uf_fn]=uf_ttop; uf_cell[uf_fn]=c; uf_fn++; uf_root[c]=c; uf_rank[c]=0; for(int j=0;j<8;++j){ int n=c+off8[j]; if(!g[n] && uf_root[n]>=0) uf_link_tr(c,n); } }
static void uf_del(int c){ --uf_fn; int base=uf_frame[uf_fn]; while(uf_ttop>base){ --uf_ttop; int rb=uf_tc[uf_ttop]; int ra=uf_root[rb]; uf_root[rb]=rb; if(uf_tr[uf_ttop]) uf_rank[ra]--; } uf_root[c]=-1; }
static void uf_rebuild(void){ uf_ttop=0; uf_fn=0; for(int c=0;c<N;++c){ uf_root[c]=g[c]?-1:c; uf_rank[c]=0; } for(int c=0;c<N;++c) if(!g[c]) for(int j=0;j<8;++j){ int n=c+off8[j]; if(n>=0 && n<N && !g[n]) uf_link0(c,n); } }
static int query_seg_cycle(int p,int dd,int len){ ++uf_qgen; for(int k=0,q=p;k<len;++k){ q+=dd; for(int j=0;j<8;++j){ int n=q+off8[j]; if(!g[n] && uf_root[n]>=0){ int r=uf_find(n); if(uf_qseen[r]==uf_qgen) return 1; uf_qseen[r]=uf_qgen; } } } return 0; }
...
static int dfs(int p, int remaining, int depth) {
    if (inc_conn && remaining == total_free - 1) uf_rebuild();
    ...
    for (int d = 0; d < 4; ++d) {
        ...
        int seg_cyc = (inc_conn && rem2 != 0) ? query_seg_cycle(p, dd, len) : 1;
        for (int k = 0, q = p; k < len; ++k) { q += dd; mark(q); }
        int rok;
        if (rem2 == 0) rok = 1;
        else if (!cheap_ok(c, rem2)) rok = 0;
        else if (inc_conn && !seg_cyc) { rok = 1; ++rl_skipped; }
        else { rok = reach_local(p + dd, dd, len, c, rem2); ++rl_taken; }
    ...
    for (int i0 = 0; i0 < nc; ++i0) {
        int i = (i0 + rot) % nc;
        int dd = delta[cand[i].dir], len = cand[i].len, c = cand[i].endp;
        for (int k = 0, q = p; k < len; ++k) { q += dd; mark(q); if (inc_conn) uf_add(q); }
        path[path_len++] = DCH[cand[i].dir];
        int r = dfs(c, remaining - len, depth + (nc > 1 ? 1 : 0));   // depth = 分支决策数，强制步不计
        if (r) return r;
        --path_len;
        for (int k = 0, back = c; k < len; ++k, back -= dd) { if (inc_conn) uf_del(back); unmark(back); }
    }
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    { int o8[8] = {-W-1,-W,-W+1,-1,1,W-1,W,W+1}; for (int i=0;i<8;++i) off8[i]=o8[i]; }
    inc_conn = getenv("INCCONN") ? atoi(getenv("INCCONN")) : 0;
    ...
    uf_root = malloc(sizeof(int) * (size_t)N); uf_rank = malloc(sizeof(int) * (size_t)N);
    uf_frame = malloc(sizeof(int) * (size_t)N); uf_cell = malloc(sizeof(int) * (size_t)N);
    uf_qseen = calloc((size_t)N, sizeof(int));
    uf_tc = malloc(sizeof(int) * (8 * (size_t)N + 64)); uf_tr = malloc(8 * (size_t)N + 64);
