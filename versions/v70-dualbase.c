...
static void build_flow_base(int X, int slot, const unsigned char *est) {
    ...
    for (int c = 0; c < N; ++c) {
        ...
        if (col[c] == X) fadd_k(FSRC, FCELL(c), rb, AK_SRC2C, c, 0);
        else             fadd_k(FCELL(c), FSNK, rb, AK_C2SNK, c, 0);
    }
    if (prop_endcol != X) {
        fadd_k(FSRC, FDUM, 1, AK_SRC2D, 0, 0);
        for (int c = 0; c < N; ++c) if (g0[c] && col[c] != X) fadd_k(FDUM, FCELL(c), 1, AK_D2C, c, 0);
    } else {
        fadd_k(FDUM, FSNK, 1, AK_D2SNK, 0, 0);
        for (int c = 0; c < N; ++c) if (g0[c] && col[c] == X) fadd_k(FCELL(c), FDUM, 1, AK_C2D, c, 0);
    }
    for (int c = 0; c < N; ++c) {
