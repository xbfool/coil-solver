// Coil solver — v38: v35 + PROF 分层计时（量出各层的时间占比和证伪贡献）
//
// 以下是 v35 的说明。
...
// 根本不该付 probing 的钱。
// PROF=1：量出 propagate_strong 各层各花多少时间、各自证伪掉多少起点。
// v35 按「规则代价」排过序，但依据是估计的（probing 几千次 try_edge，流只有一次 max-flow）。
// 真实的时间分布和证伪贡献得量出来，才知道下一刀切哪 —— 尤其在那 6 个全局过滤完全失效的关上。
static double t_lay[6];
static long long k_lay[6], n_call2;
...
static int propagate_strong(int s) {
    ...
    double t0 = nowsec(), t1;
    int r;
    r = propagate(s);             t1 = nowsec(); t_lay[0] += t1-t0; t0=t1; if(!r){++k_lay[0];return 0;}
    r = do_flow(s);               t1 = nowsec(); t_lay[1] += t1-t0; t0=t1; if(!r){++k_lay[1];return 0;}
    r = do_probing(probe_rounds); t1 = nowsec(); t_lay[2] += t1-t0; t0=t1; if(!r){++k_lay[2];return 0;}
    r = do_flow(s);               t1 = nowsec(); t_lay[3] += t1-t0; t0=t1; if(!r){++k_lay[3];return 0;}
    r = do_probing(1);            t1 = nowsec(); t_lay[4] += t1-t0; t0=t1; if(!r){++k_lay[4];return 0;}
    r = orient_chains(s);         t1 = nowsec(); t_lay[5] += t1-t0;       if(!r){++k_lay[5];return 0;}
    return 1;
}
static void prof_dump(void) {
    if (!prof_on) return;
    static const char *nm[6] = {"传播  ","流1   ","probing","流2   ","probe2","链定向"};
    double tot = 0;
    for (int i2 = 0; i2 < 6; ++i2) tot += t_lay[i2];
    if (tot <= 0) return;
    fprintf(stderr, "PROF shard%d: %lld 次调用, 合计 %.2fs\n", shard, n_call2, tot);
    for (int i2 = 0; i2 < 6; ++i2)
        fprintf(stderr, "   %s %7.2fs (%4.1f%%)  证伪 %lld\n", nm[i2], t_lay[i2], 100*t_lay[i2]/tot, k_lay[i2]);
}
...
int main(int argc, char **argv) {
    ...
    prof_on = getenv("PROF") ? atoi(getenv("PROF")) : 0;
