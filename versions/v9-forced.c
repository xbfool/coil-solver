// Coil solver — v9: 开局先做一次边层面的约束传播，把「必然走/必然不走」预先算出来当 O(1) 剪枝
//
...
static unsigned char *fmask, *xmask;    // 每格 4 bit：该方向的边是「必选」/「已排除」
static int use_forced = 1;              // 关掉后退化成 v5，用作完备性兜底
static int start_cell = -1;             // 起点豁免（端点只用 1 条边，环模型会给它多推一条）
static int *ufp;

static int uf(int a) { while (ufp[a] != a) { ufp[a] = ufp[ufp[a]]; a = ufp[a]; } return a; }

// 边层面的约束传播，O(n) 级，开局跑一次
static void propagate(void) {
    for (int c = 0; c < N; ++c) ufp[c] = c;
    int changed = 1;
    while (changed) {
        changed = 0;
        for (int c = 0; c < N; ++c) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                if (fmask[c] & (1 << d)) { ++f; continue; }
                if (xmask[c] & (1 << d)) continue;
                if (uf(c) == uf(c + delta[d])) {            // 用了就成环
                    xmask[c] |= 1 << d; xmask[c + delta[d]] |= 1 << (d ^ 2);
                    changed = 1; continue;
                }
                ++u; umask |= 1 << d;
            }
            if (f >= 2 && u) {                              // 已定满，其余排除
                for (int d = 0; d < 4; ++d) if (umask & (1 << d)) {
                    xmask[c] |= 1 << d; xmask[c + delta[d]] |= 1 << (d ^ 2);
                }
                changed = 1;
            } else if (f + u == 2 && u) {                   // 只剩这些，必选
                for (int d = 0; d < 4; ++d) if (umask & (1 << d)) {
                    fmask[c] |= 1 << d; fmask[c + delta[d]] |= 1 << (d ^ 2);
                    int ra = uf(c), rb = uf(c + delta[d]);
                    if (ra != rb) ufp[ra] = rb;
                }
                changed = 1;
            }
...
static int dfs(int p, int remaining) {
    ...
    for (int d = 0; d < 4; ++d) {
        ...
        while (g[c + dd]) {
            ...
            c += dd; mark(c); ++len;
            if (use_forced) {
                // 从 prev 走到 c 用了 prev 的 dd 方向边
                if (prev != start_cell && (xmask[prev] & (1 << d))) { bad = 1; break; }
                if (c != start_cell && (xmask[c] & (1 << (d ^ 2)))) { bad = 1; break; }
                // prev 若是「穿过」格（不是本次滑动的第一格前的位置 p），它进出两条边就定了
                if (prev != p && prev != start_cell) {
                    int used = (1 << d) | (1 << (d ^ 2));
                    if (fmask[prev] & ~used) { bad = 1; break; }
                }
            }
        }
        int rem2 = remaining - len;
        if (bad) { for (int i = 0, back = c; i < len; ++i, back -= dd) unmark(back); continue; }
...
int main(int argc, char **argv) {
    ...
    fmask = calloc(N, 1); xmask = calloc(N, 1); ufp = malloc(sizeof(int) * (size_t)N);
    propagate();
