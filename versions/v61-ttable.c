// Coil solver — v61: v56 + 置换表（Zobrist 搜穷缓存，TTGB 开表）
//
// 以下是 v53 的说明。
...
// ===== 置换表（v61）=====
static unsigned long long *zvis, zcur, zsalt;
static unsigned long long *tt; static unsigned long long ttmask; static int tt_on, tt_gate;
static long long tt_hit, tt_store_n;
static unsigned long long zrng(unsigned long long *st) { *st ^= *st << 13; *st ^= *st >> 7; *st ^= *st << 17; return *st; }
...
static inline void unmark(int c) {
    zcur ^= zvis[c];               // 已访问 -> 未访问（严格 LIFO 回滚）
    if (live_end >= 0 && tcand[c]) ++live_end;
    g[c] = 1; ++cnt[col[c]];
    for (int d = 0; d < 4; ++d) {
        int n = c + delta[d];
...
static int dfs(int p, int remaining, int depth) {
    ...
    unsigned long long tkey = 0; long long tnodes0 = 0;
    if (tt_on && tt_gate && nc > 1) {
        tkey = zcur ^ zvis[p] ^ zsalt;
        if (tt[tkey & ttmask] == (tkey | 1ULL)) { ++tt_hit; return 0; }
        tnodes0 = nodes;
    }
    for (int i = 1; i < nc; ++i)
    ...
    if (tt_on && tt_gate && tkey && nodes - tnodes0 >= (getenv("TTMIN") ? atoll(getenv("TTMIN")) : 256)) {
        tt[tkey & ttmask] = tkey | 1ULL;
        ++tt_store_n;
    }
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    {
        zvis = malloc(sizeof(unsigned long long) * (size_t)N);
        unsigned long long st = 0x9E3779B97F4A7C15ULL;
        for (int c = 0; c < N; ++c) zvis[c] = zrng(&st);
        int gb = getenv("TTGB") ? atoi(getenv("TTGB")) : 0;
        if (gb > 0) {
            size_t entries = ((size_t)gb << 30) / 8;
            size_t p2 = 1; while (p2 * 2 <= entries) p2 *= 2;
            tt = calloc(p2, 8);
            if (tt) { ttmask = p2 - 1; tt_on = 1; }
        }
    }
  ...
  child_work:;
    ...
    for (int i0 = 0; i0 < keep; ++i0) {
        ...
        zsalt = zvis[s] * 0x2545F4914F6CDD1DULL;
        tt_gate = 1;
        live_end = -1; tc_on = 0; ntc_low = 0;
        if (use_flow && tcand_for == s) {
            live_end = 0;
        ...
        if (getenv("TREELOG")) fprintf(stderr, "T3 shard%d r%d #%d cell=(%d,%d) sc=%d nodes=%lld r=%d tthit=%lld ttstore=%lld\n",
            shard, round3, i0, s % W - 1, s / W - 1, sc[i], nodes, t3r, tt_hit, tt_store_n);
    ...
    if (tt_on) fprintf(stderr, "TT hit=%lld store=%lld\n", tt_hit, tt_store_n);
    fprintf(stderr, "no solution found\n");
    return 1;
}
