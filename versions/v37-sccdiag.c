// Coil solver — v37: v35 + SCCSTAT 诊断（观察残量图 SCC 分布，查过滤为何在偶数关失效）
//
// 以下是 v35 的说明。
...
#define OKAB(c) (rstA < 0 || blk[c] == rstA || blk[c] == rstB)
    ...
    // ---- SCCSTAT=1：直接观察残量图的 SCC 分布 ----
    // v36 的教训是「不看数据直接改模型会白干」。end_ok 的判据是
    //     used || fscc[哑点] == fscc[格子]
    // 所以**过滤失效** 等价于 **哑点所在的那个 SCC 吞掉了大部分格子**。
    // 要分清的是两种情况：(a) 整个残量图就是一个巨型 SCC（度约束层面真的不受约束）；
    // (b) 图本身碎成很多块，但哑点恰好落在最大的那块里（那就是 hub 扇出的问题，可修）。
    // 度约束松弛的解是「2-因子」（每格度 2 的子图），它允许拆成一堆不相交的圈，
    // 而哈密顿路径要求单一路径 —— 缺的正是**连通性**。先看这条约束到底 binding 不 binding：
    // 把当前可行流还原成 2-因子（必用边 + 流里用掉的边），数它有几个连通块。
    //   · 只有 1~2 块 => 连通性天然满足，加 subtour 约束不会有收益；
    //   · 几百块       => 松弛解散成一地碎圈，连通性就是缺失的那条约束。
    if (getenv("SCCSTAT")) {
        int *uf = malloc(sizeof(int) * (size_t)N);
        for (int c = 0; c < N; ++c) uf[c] = c;
        int deg2[3] = {0, 0, 0};
        int nedge = 0;
        for (int c = 0; c < N; ++c) {
            if (!g0[c]) continue;
            for (int d = 0; d < 4; ++d) {
                int n = c + delta[d];
                if (!g0[n] || n < c) continue;
                int used = 0;
                if (estate[c * 4 + d] == 1) used = 1;
                else if (!col[c]) { int e = fmap[c * 4 + d]; if (e >= 0 && fcap[e] <= 0) used = 1; }
                else { int e = fmap[n * 4 + (d ^ 2)]; if (e >= 0 && fcap[e] <= 0) used = 1; }
                if (!used) continue;
                ++nedge;
                int a = c, b = n;
        ...
        int ncomp = 0, nfree2 = 0;
        for (int c = 0; c < N; ++c) if (g0[c]) { ++nfree2; int a = c; while (uf[a] != a) a = uf[a]; if (a == c) ++ncomp; }
        (void)deg2;
        fprintf(stderr, "2-因子: 用了 %d 条边, 连通块 %d 个 (自由格 %d)\n", nedge, ncomp, nfree2);
        free(uf);
    }
    if (getenv("SCCSTAT")) {
        int *sz = calloc((size_t)nscc, sizeof(int));
        for (int i2 = 0; i2 < fnodes; ++i2) if (fscc[i2] >= 0) ++sz[fscc[i2]];
        int cellscc = 0, biggest = 0, bigid = -1;
        for (int i2 = 0; i2 < nscc; ++i2) if (sz[i2] > biggest) { biggest = sz[i2]; bigid = i2; }
        // 只数「格子」节点，别把 SRC/SNK/哑点算进去
        int *csz = calloc((size_t)nscc, sizeof(int));
        int nfree = 0;
        for (int c = 0; c < N; ++c) if (g0[c]) { ++nfree; if (fscc[FCELL(c)] >= 0) ++csz[fscc[FCELL(c)]]; }
        for (int i2 = 0; i2 < nscc; ++i2) if (csz[i2] > 0) ++cellscc;
        int dA = fscc[FDUM], dB = fscc[FDUM2];
        int inA = dA >= 0 ? csz[dA] : 0, inB = dB >= 0 ? csz[dB] : 0;
        int maxc = 0; for (int i2 = 0; i2 < nscc; ++i2) if (csz[i2] > maxc) maxc = csz[i2];
        fprintf(stderr,
            "SCC: 总块 %d, 含格块 %d | 最大含格块 %d/%d = %.1f%% | "
            "哑点A块含格 %d (%.1f%%)%s | 哑点B块含格 %d (%.1f%%)%s | 自由格 %d\n",
            nscc, cellscc, maxc, nfree, 100.0 * maxc / nfree,
            inA, 100.0 * inA / nfree, (dA == bigid ? " <-最大块" : ""),
            inB, 100.0 * inB / nfree, (dB == bigid ? " <-最大块" : ""), nfree);
        free(sz); free(csz);
    }
...
// SHATTER=1：巨型 SCC 是不是「只差一点点种子」就能打碎？
// 失败关的残量图整个是一个含格 SCC（100%），成功关碎成近两千块（最大 3%）—— 断崖式，没有中间态。
// 这里逐条试着**固定一条未定边**（必用 / 禁用各试一次）再重跑流，看含格块数会不会塌下来。
//   · 一条边就能打碎  => 只差种子，那就该在全局阶段做 probing 去造种子；
//   · 一条边打不碎    => 是这些实例在度约束松弛下真的自由，得换更强的松弛（比如加连通性）。
static void shatter_test(int limit) {
    memset(estate, 0, (size_t)N * 4);
    for (int c = 0; c < N; ++c) end_ok[c] = 1;
    filter_endpoints();
    int base = gfilter_kept;
    fprintf(stderr, "基线: 端点候选 %d\n", base);
    int tried = 0, best = base, bestv = -1;
    for (int c = 0; c < N && tried < limit; ++c) {
        if (!g0[c] || col[c]) continue;
        for (int d = 0; d < 4 && tried < limit; ++d) {
            int n = c + delta[d];
            if (!g0[n] || estate[c * 4 + d] != 0) continue;
            for (int v = 1; v <= 2; ++v) {
                unsigned char sa = estate[c * 4 + d], sb = estate[n * 4 + (d ^ 2)];
                estate[c * 4 + d] = estate[n * 4 + (d ^ 2)] = (unsigned char)v;
                int ok = filter_endpoints();
                if (ok && gfilter_kept < best) { best = gfilter_kept; bestv = v; }
                if (tried < 6)
                    fprintf(stderr, "  固定边(%d,%d)方向%d=%s -> %s端点候选 %d\n",
                            c % W - 1, c / W - 1, d, v == 1 ? "必用" : "禁用",
                            ok ? "" : "流不可行! ", ok ? gfilter_kept : -1);
                estate[c * 4 + d] = sa; estate[n * 4 + (d ^ 2)] = sb;
            }
            ++tried;
        }
    }
    fprintf(stderr, "试了 %d 条边: 最好把端点候选从 %d 压到 %d (%s)\n",
            tried, base, best, bestv < 0 ? "没压动" : (bestv == 1 ? "靠必用" : "靠禁用"));
}
...
int main(int argc, char **argv) {
    ...
    if (getenv("SHATTER")) { shatter_test(atoi(getenv("SHATTER"))); return 0; }
