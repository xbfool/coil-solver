...
W=260
LOG=/tmp/bd767.log; : > "$LOG"
WIN=/tmp/bd767.win; rm -f "$WIN"
J=${1:-18}; LIMIT=${2:-3600}
DONE=/tmp/done87.txt
while read -r _ cell _; do
  ...
  (
    S=$(date +%s)
    CHAIN=0 FLOW=0 DIRLAYER=0 PROPDEPTH=999999 SEEDMODE=0 PINSTART=$X,$Y JOBS=1 \
      timeout "$LIMIT" "$BIN" "$BOARD" > "/tmp/bd_$cell.txt" 2>/dev/null
    T=$(( $(date +%s) - S ))
    if [ -s "/tmp/bd_$cell.txt" ] && "$CHECK" "$BOARD" "/tmp/bd_$cell.txt" >/dev/null 2>&1; then
      cp "/tmp/bd_$cell.txt" "$BANK/767.sol"
      echo "($X,$Y): ${T}s **出解!!**" >> "$LOG"
      touch "$WIN"
    else
      echo "($X,$Y): ${T}s 无" >> "$LOG"
