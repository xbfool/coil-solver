...
W=260
LOG=/tmp/bd2_767.log; : > "$LOG"
WIN=/tmp/bd2_767.win; rm -f "$WIN"
J=${1:-18}; LIMIT=${2:-3600}
DONE=/tmp/done87.txt
while read -r _ cell _; do
  [ -f "$WIN" ] && break
  X=$(( cell % W - 1 )); Y=$(( cell / W - 1 ))
  grep -q "^$X,$Y$" "$DONE" 2>/dev/null && continue   # 上一轮已秒杀的跳过（它们是快树，结论可靠）
  (
    S=$(date +%s)
    CHAIN=0 FLOW=0 DIRLAYER=0 PDEVERY=20 BJ=20 SEEDMODE=0 PINSTART=$X,$Y JOBS=1 \
      timeout "$LIMIT" "$BIN" "$BOARD" > "/tmp/bd2_$cell.txt" 2>/dev/null
    T=$(( $(date +%s) - S ))
    if [ -s "/tmp/bd2_$cell.txt" ] && "$CHECK" "$BOARD" "/tmp/bd2_$cell.txt" >/dev/null 2>&1; then
      cp "/tmp/bd2_$cell.txt" "$BANK/767.sol"
      echo "($X,$Y): ${T}s **出解!!**" >> "$LOG"
      touch "$WIN"
    else
      echo "($X,$Y): ${T}s 无" >> "$LOG"
