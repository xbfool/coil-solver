...
set -u
BOARD="$1"; SURV="$2"; OUT="$3"; PAR="${4:-13}"; BOX="${5:-1800}"; BIN="${6:-bin/king}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CK="$ROOT/../coilbench/coil_check/check"
cd "$ROOT"; mkdir -p "$OUT"
W=$(head -c 30 "$BOARD" | grep -oE 'x=[0-9]+' | grep -oE '[0-9]+'); WW=$((W+2))
ulimit -s unlimited 2>/dev/null

# SURV="-" 表示 $OUT/pins.txt 已备好(x,y 每行一个),跳过 FUNNELDUMP 转换
if [ "$SURV" != "-" ]; then
cat "$SURV".* | grep -v '^#' | sort -un | while read -r s; do
...
grind_one() {
    ...
    timeout "$BOX" env PINSTART="$pin" STATS=1 RESTART=1 SWEEP="${PINSWEEP:-3000000}" JOBS=1 \
