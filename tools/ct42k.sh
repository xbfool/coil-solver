...
set -uo pipefail
BIN=/mnt/d/works/coil-solver/bin/v77b
...
J=${1:-18}; LIMIT=${2:-3600}
DONE=/tmp/done87.txt
cut -d' ' -f2 /tmp/cand_767.txt | sort > /tmp/_b87.txt
cut -d' ' -f2 /tmp/abl_CHAINTURN.txt | sort | comm -23 - /tmp/_b87.txt | sed 's/^/OK /' > /tmp/extras_ct.txt
echo "差集 $(wc -l < /tmp/extras_ct.txt) 个"
while read -r _ cell _; do
  ...
  (
    ...
    CHAIN=0 FLOW=1 DIRLAYER=0 PDEVERY=20 BJ=20 PINSTART=$X,$Y JOBS=1 \
      timeout "$LIMIT" "$BIN" "$BOARD" > "/tmp/ctp_$cell.txt" 2>/dev/null
    T=$(( $(date +%s) - S ))
    if [ -s "/tmp/ctp_$cell.txt" ] && "$CHECK" "$BOARD" "/tmp/ctp_$cell.txt" >/dev/null 2>&1; then
      cp "/tmp/ctp_$cell.txt" "$BANK/767.sol"
      echo "($X,$Y): ${T}s **出解!!**" >> "$LOG"
      touch "$WIN"
    else
      echo "($X,$Y): ${T}s 无" >> "$LOG"
...
done < /tmp/extras_ct.txt
wait 2>/dev/null
sort "$LOG"
[ -f "$WIN" ] && echo "== 767 攻陷 ==" || echo "== 42k 差集全空 => flow 毒边实锤 =="
